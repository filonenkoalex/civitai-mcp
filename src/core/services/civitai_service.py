...
class CivitaiService:
    ...
    async def generate_and_download(
        ...
        while True:
            if time.time() - start_time > timeout:
                raise TimeoutError(f"Image generation did not complete within {timeout} seconds")
            ...
            if result and isinstance(result, list) and len(result) > 0:
                ...
                if result_item.get('available') and result_item.get('blobUrl'):
                    ...
                    image_data = await self._download_image(blob_url)
    ...
    async def _download_image(self, blob_url: str) -> bytes:
        """
